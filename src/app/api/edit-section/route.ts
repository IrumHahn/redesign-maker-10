...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = [
      ...
      `이 섹션이 맡은 역할(레이아웃 참고용이며 글자로 그리지 않는다): ${section.purpose || ""}`,
      ...
      "한글 정확도: 이미지 안의 한글을 한 글자도 틀리지 않게 그린다. 받침, 모음, 자음 하나가 달라져도 다른 단어가 된다. 기존 이미지의 글자를 다시 그릴 때도 철자를 그대로 유지한다.",
      "서체 규칙: 이미지 안의 모든 한글, 영문, 숫자는 Pretendard 하나로만 쓴다. Pretendard는 획 굵기가 고른 현대적인 한국어 산세리프이고, 자소가 네모틀에 꽉 차며 끝이 직각으로 잘린 깔끔한 서체다. 글자를 새로 그리거나 고칠 때도 서체를 바꾸지 않는다. 정보 위계는 굵기(Regular / SemiBold / ExtraBold)와 크기로만 만들고, 명조·바탕·붓글씨·손글씨·기울임체·장식체를 한 글자도 섞지 않는다.",
      "모바일 가독성 규칙: 정보를 줄이는 대신 글자를 시원하게 키워 스마트폰에서 한 손에 들고 봐도 편히 읽히게 한다. 대제목은 화면 가로를 거의 채울 만큼 아주 크게, 소제목과 라벨은 대제목의 절반쯤, 설명문도 작아지지 않게 또렷하게 쓴다. 깨알 같은 글씨와 빽빽한 표는 피한다.",
      "금지: 구매 버튼, 장바구니 버튼, CTA 버튼을 이미지 안에 그리지 않는다.",
      "금지: 메뉴 바, 탭, 링크, 검색창, 하단 아이콘 메뉴처럼 누를 수 있어 보이는 UI를 그리지 않는다. 상세페이지는 이미지라서 클릭되지 않는다.",
      "금지: 평점 숫자, 후기 개수, 판매량, 순위 배지처럼 시점마다 바뀌는 숫자를 그리지 않는다. 여러 플랫폼에 쓰이고 수치는 계속 변해서 곧 거짓이 된다.",
      "허용: 후기 카드의 별 아이콘은 한국 상세페이지 관습이므로 그대로 두거나 또렷하게 그려도 된다. 다만 별 옆에 점수나 후기 개수를 숫자로 적지 않는다.",
      "삭제 방법: 첨부 이미지에서 어떤 요소를 뺄 때 흐리게 처리하거나 모자이크, 반투명 덮개, 색 블록으로 가리지 않는다. 가린 자국이 남으면 실패다. 요소를 완전히 없애고 그 자리를 주변 배경과 카드 색으로 자연스럽게 메운 뒤, 남은 공간은 문장을 더 크게 키우거나 여백을 정리해 원래부터 없었던 것처럼 만든다.",
      "금지: '상세페이지에서 확인 가능', '정해진 규정에 따라 처리', '빠르고 안전하게 배송'처럼 읽어도 정보가 남지 않는 문구를 그리지 않는다. 구체적인 사실이 없으면 그 칸을 비우거나 빼고 레이아웃을 다시 짠다.",
      "금지: 기획 메모나 제작 지시문을 이미지에 그리지 않는다. '표기', '노출', '영역', '섹션', '원본', '각주', '~하지 마세요' 같은 표현과 퍼센트·비율·글자 크기 규칙 같은 디자인 설명은 소비자용 문구가 아니다. 모든 문구는 소비자에게 직접 말하는 문장이어야 한다.",
