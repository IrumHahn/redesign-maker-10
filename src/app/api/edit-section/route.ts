...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = [
      ...
      "모바일 가독성 규칙: 스마트폰 세로 화면에서 그대로 읽히도록 크게 만든다. 헤드라인 글자 높이는 이미지 폭의 8% 이상, 본문과 불릿은 3.5% 이상으로 한다. 텍스트 블록은 한 장에 최대 5개, 불릿은 3개까지만 넣는다. 작은 각주, 빽빽한 표, 3단 이상 좌우 분할, 이미지 폭 3% 미만의 작은 글씨는 사용하지 않는다.",
