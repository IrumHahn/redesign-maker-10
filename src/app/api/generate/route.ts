...
function restructureAnalysisPrompt(payload: AnalysisPayload, modelInfo: ReturnType<typeof modelMeta>) {
  return [
    ...
    "- 근거가 없으면 문장을 지어내지 말고 그 항목을 빈 값으로 둘 것",
    "- 규제 리스크가 있으면 단정 표현을 피하고 안전한 표현으로 완화할 것",
    "",
    "# 카피 작성 원칙 (가장 중요)",
    "너는 원본을 점검하는 분석가가 아니라 이 제품을 파는 카피라이터다. 원본은 사실을 확인하는 재료일 뿐, 설명할 대상이 아니다.",
    "모든 문구는 제품을 사려는 소비자에게 직접 말하는 문장이어야 한다.",
    "너 자신에게 하는 지시, 기획 메모, 화면 구성 설명, 제작 규칙은 절대 문구로 쓰지 않는다.",
    "다음 단어가 들어가면 실패다: 표기, 노출, 영역, 섹션, 원본, 각주, 상세페이지, 만들지 마세요.",
    "",
    "# 감사 보고서 말투 금지 (가장 흔한 실패)",
    "원본에 무엇이 있고 없는지 보고하는 문장은 카피가 아니다. 아래 문형이 하나라도 나오면 실패다.",
    "- '~ 확인', '~ 확인했어요', '~ 볼 수 있어요' 처럼 확인 결과를 알리는 말",
    "- '~ 보세요', '함께 보세요', '신중하게 보세요' 처럼 페이지 보는 법을 지시하는 말",
    "- '~ 다는 말이 있어요', '~ 는 의견도 있어요' 처럼 후기를 요약해서 전하는 말",
    "- '~ 문구는 보이지 않아요', '~ 는 나와 있지 않아요' 처럼 없는 것을 알리는 말",
    "- '구매 기준', '확인 근거', '착용 의견' 처럼 제작자가 붙인 분류 이름",
    "정보가 없으면 그 항목을 빈 값으로 둔다. 없다는 사실을 문장으로 쓰면 절대 안 된다.",
    "",
    "# 절대 쓰지 않는 문구 (위반하면 결과물 폐기)",
    "- 후기 개수, 평균 별점, 평점, 판매량, 누적 판매, 순위, 재구매율처럼 시점마다 바뀌는 수치. 이 페이지는 여러 플랫폼에 쓰이고 수치는 계속 변해서 곧 거짓이 된다",
    "- \'상세페이지에서 확인 가능\', \'정해진 규정에 따라 처리\', \'빠르고 안전하게 배송\', \'꾸준히 다시 찾는 고객\'처럼 읽어도 아무 정보가 남지 않는 문구",
    "- \'자주묻는질문\', \'배송안내\', \'교환반품\', \'상담문의\', \'더보기\'처럼 누르는 메뉴 이름. 상세페이지는 이미지라서 클릭되지 않는다",
    "",
    "잘못된 예 → 올바른 예",
    "- \'기술명과 사진으로 구매 전 불안을 줄여요\' → \'흔들림 잡아주는 안정 구조\'",
    "- \'옆면·뒤꿈치·상단·바닥을 함께 보세요\' → \'구석구석 단단하게 잡아줍니다\'",
    "- \'편하다는 말과 발볼이 좁다는 말이 함께 있어요\' → \'하루 종일 신어도 편했다는 발\'",
    "- \'아주 편하다는 의견 확인\' → \'첫 착화부터 부드러운 쿠션\'",
    "- \'구체적인 기간·비용 문구는 보이지 않아요\' → (근거가 없으면 이 항목을 아예 만들지 않는다)",
    "- \'판매자 :: 아식스코리아로 확인\' → \'아식스코리아 공식 판매\'",
    "- \'구매 전 확인할 근거 모음\' → \'민감 피부도 편안하게\'",
    "- \'13,314개의 후기가 증명하는 만족도\' → \'직접 써본 사람들의 이야기\'",
    "- \'평균 별점 4.8점\' → \'자연스러운 톤업이라 매일 손이 간다는 후기\'",
    "- \'민감성 피부 대상 완료 표기\' → \'민감성 피부 테스트 완료\'",
    "- \'전성분: 상세페이지에서 확인 가능\' → (원본에 성분 정보가 없으면 이 항목을 만들지 않는다)",
    "- \'교환/반품: 정해진 규정에 따라 처리\' → \'수령 후 7일 이내 교환 가능\' (원본에서 확인된 조건일 때만)",
    ...
    '{ "section_id": "S1", "eyebrow": "제품 카테고리·특징 라벨 12자 이내(예: 차량용 쓰레기봉투). 섹션 역할 이름 금지", "headline": "굵은 대제목 30자 이내, 2~3줄로 끊어 읽히게", "subheadline": "설명 45자 이내", "points": [{"label": "굵은 소제목 10자 이내", "desc": "구체적인 사실 설명 30자 이내"}], "extras": ["제품의 구체적 사실을 담은 짧은 라벨 8자 이내. 메뉴 이름 금지"], "evidence": "원본에서 확인된 근거만" }',
    "points는 최대 3개, extras는 최대 5개까지 쓴다. 원본에서 확인되는 만큼만 쓰고, 확인되지 않으면 개수를 줄이거나 빈 배열로 둔다. 칸을 채우려고 뜻 없는 문구를 만들면 실패다.",
    "섹션 역할: S1 히어로 / S2 문제공감 체크리스트 / S3 베네핏 3개 / S4 USP 차별점 / S5 근거·신뢰 / S6 사용법 / S7 후기(수치 없이 사용감 문장만) / S8 배송·교환 안내(원본에서 확인된 조건만)",
...
function buildSections(
  ...
  return plans.map((plan) => {
    ...
    const promptText = [
      ...
      "금지: 위에 적힌 섹션 이름과 목적은 레이아웃을 잡기 위한 내부 분류다. 이미지 안에 문구로 쓰지 않는다. '표기', '노출', '영역', '섹션', '원본', '각주' 같은 기획 용어와 제작 지시문, 퍼센트·비율·글자 크기 규칙 같은 디자인 설명도 절대 글자로 그리지 않는다. 위에 지정된 문구 외에 새 글자를 만들어 넣지 않는다.",
      TYPOGRAPHY_RULE,
...
const META_COPY_PATTERNS = [
  ...
  // 디자인 지시문이 카피로 새어 나온 경우 ("소제목과 라벨은 4.5% 이상, 설명문은 3.5% 이상")
  // 성분 함량 표기(2% 함유)는 정당한 정보라서 퍼센트 자체는 막지 않는다.
  /소제목|설명문|대제목|헤드라인|서브헤드/,
  /글자\s*(높이|크기)|이미지\s*폭|폰트/
];

/**
 * 후기 개수나 평점처럼 시점마다 바뀌는 수치는 어느 모드에서도 이미지에 넣지 않는다.
 * 같은 상세페이지를 여러 플랫폼에 올리고 수치는 계속 변하기 때문에 곧 거짓이 된다.
 */
...
/** 읽어도 정보가 남지 않는 빈말은 이미지에 그려지기 전에 버린다. */
const HOLLOW_COPY_PATTERNS = [
  /확인\s*(가능|하세요|해\s*보세요|바랍니다)/,
  /규정에\s*따라|절차에\s*따라|규정\s*준수/,
  /빠르고\s*안전하게|안전하게\s*배송|신속\s*정확/,
  /자세한\s*(내용|사항|정보)|아래에서\s*확인|하단\s*참고/,
  /꾸준히\s*다시\s*찾는|많은\s*분들이\s*선택|믿고\s*구매/
];

/** 짧은 라벨이 누르는 메뉴이거나 숫자 배지면 버린다. 상세페이지 이미지는 클릭되지 않는다. */
const NAV_LABEL_PATTERNS = [
  // "13314개", "4.8점", "1위"처럼 숫자만 있는 배지
...
function rejectReason(value: string, kind: "text" | "label") {
  if (isMetaCopy(value)) return "기획 메모";
  if (VOLATILE_CLAIM_PATTERNS.some((pattern) => pattern.test(value))) return "변동 수치";
  if (HOLLOW_COPY_PATTERNS.some((pattern) => pattern.test(value))) return "빈말";
  if (kind === "label" && NAV_LABEL_PATTERNS.some((pattern) => pattern.test(value))) return "메뉴 라벨";
  return "";
}

/** 소비자에게 보여줄 문구가 아니라고 판단되면 버린다. 이미지에 그려지는 것보다 비는 편이 낫다. */
function consumerText(value: unknown, kind: "text" | "label" = "text") {
  const trimmed = text(value);
  if (!trimmed) return "";
  const reason = rejectReason(trimmed, kind);
  if (reason) {
    console.warn(`[generate] ${reason}으로 판단해 카피에서 제외: ${trimmed}`);
    return "";
  }
  return trimmed;
}
...
function readSectionCopy(match: Record<string, unknown>, maxPoints = 4, maxExtras = 5): SectionCopy {
  const points = Array.isArray(match.points)
    ? match.points
        .map((point) => {
          if (typeof point === "string") return { label: consumerText(point, "label"), desc: "" };
          const record = point as Record<string, unknown>;
          const rawDesc = text(record?.desc);
          const desc = consumerText(rawDesc);
          // 설명이 빈말로 걸러졌으면 라벨만 남겨도 정보가 없으니 포인트 전체를 버린다.
          // ("전성분 :: 상세페이지에서 확인 가능", "재구매 :: 꾸준히 다시 찾는 고객들의 후기")
          if (rawDesc && !desc) return { label: "", desc: "" };
          return {
            label: consumerText(record?.label, "label"),
            desc
          };
        })
  ...
  const extras = Array.isArray(match.extras)
    ? match.extras.map((extra) => consumerText(extra, "label")).filter((extra) => extra.length > 0).slice(0, maxExtras)
    : [];

  return {
    eyebrow: consumerText(match.eyebrow, "label"),
